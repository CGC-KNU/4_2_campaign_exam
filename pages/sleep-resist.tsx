...
import { fetchSleepLeaderboard, getPersonalBest, saveSleepRecord, updatePersonalBest } from '../lib/sleep-resist/storage'
...
export default function SleepResistPage() {
  ...
  const [overallRecords, setOverallRecords] = useState<SleepResistRecord[]>([])
  const [todayRecords, setTodayRecords] = useState<SleepResistRecord[]>([])
  ...
  useEffect(() => {
    ...
    async function loadLeaderboard() {
      ...
      try {
        const leaderboard = await fetchSleepLeaderboard()
        if (!cancelled) {
          setOverallRecords(leaderboard.overallRecords)
          setTodayRecords(leaderboard.todayRecords)
        }
      } catch (error) {
        if (!cancelled) {
  ...
  async function saveRanking() {
    ...
    try {
      ...
      setOverallRecords(result.overallRecords)
      setTodayRecords(result.todayRecords)
      setSaveMessage(`${result.record.nickname} 이름으로 기록을 남겼어요.`)
