...
import { fetchSleepLeaderboard, getPersonalBest, saveSleepRecord, sortSleepRecords, updatePersonalBest } from '../lib/sleep-resist/storage'
import type { SleepEvent, SleepGameState, SleepResistRecord } from '../lib/sleep-resist/types'

const CAMPAIGN_END_DATE_KEY = '2026-04-25'
const OVERALL_LEADERBOARD_LIMIT = 10
const TODAY_LEADERBOARD_LIMIT = 30
...
export default function SleepResistPage() {
  ...
  const [leaderboardRecords, setLeaderboardRecords] = useState<SleepResistRecord[]>([])
  ...
  useEffect(() => {
    ...
    async function loadLeaderboard() {
      ...
      try {
        const records = await fetchSleepLeaderboard()
        if (!cancelled) {
          setLeaderboardRecords(records)
        }
      } catch (error) {
        if (!cancelled) {
          setLeaderboardError(error instanceof Error ? error.message : '랭킹을 불러오지 못했습니다.')
        }
      } finally {
        if (!cancelled) {
  ...
  const overallRecords = useMemo(
    () => sortSleepRecords(leaderboardRecords).slice(0, OVERALL_LEADERBOARD_LIMIT),
    [leaderboardRecords],
  )
  const todayRecords = useMemo(
    () => sortSleepRecords(leaderboardRecords.filter((record) => record.dateKey === todayKey)).slice(0, TODAY_LEADERBOARD_LIMIT),
    [leaderboardRecords, todayKey],
  )
  const currentRecords = leaderboardMode === 'overall' ? overallRecords : todayRecords
  const personalBest = useMemo(() => getPersonalBest(), [game.status])
  ...
  async function saveRanking() {
    ...
    try {
      const record = await saveSleepRecord({
      ...
      setSaveMessage(saveError instanceof Error ? saveError.message : '랭킹 저장에 실패했습니다.')
    }
  }
        ...
        <section className="card sleep-panel">
          ...
          <div className="sleep-rank-list">
            {isLeaderboardLoading ? (
              <p className="sleep-empty-rank">랭킹을 불러오는 중이에요...</p>
            ) : leaderboardError ? (
              <p className="sleep-empty-rank">{leaderboardError}</p>
          ...
          <p className="sleep-rank-note">
            {leaderboardMode === 'overall'
              ? '전체 랭킹은 누적 최고 기록 기준 상위 10개만 보여줍니다.'
              : '오늘의 랭킹은 오늘 등록된 기록 기준으로 30위까지만 보여줍니다.'}
          </p>
        </section>
      </section>
    </div>
